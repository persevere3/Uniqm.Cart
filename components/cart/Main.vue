        ...
        <li v-for="item in pageFilterProducts" :key="item.ID" >
          ...
          <div class="content">
            ...
            <template v-if="item.priceType === 'onePrice'">
              <div class="price origin" :class="{opacity0 : parseInt(item.Price) < 0}">NT$ {{ useNumberThousands(item.Price) }}</div>
              <div class="price">NT$ {{ useNumberThousands(item.NowPrice) }}</div>
            </template>
            <template v-else>
              <template v-if="item.selectSpecItem && item.selectSpecItem.ID">
                <div class="price origin" :class="{opacity0 : parseInt(item.selectSpecItem.ItemPrice) < 0}">NT$ {{ useNumberThousands(item.selectSpecItem.ItemPrice) }}</div>
