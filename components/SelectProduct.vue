  ...
  <div class="selectProduct">
    <div class="background">
      ...
      <div class="picContent">
        ...
        <div class="content">
            ...
            <template v-if="selectProduct.priceType === 'onePrice'">
              <div class="price origin" v-if="parseInt(selectProduct.Price) > -1">NT$ {{ useNumberThousands(selectProduct.Price) }}</div>
              <div class="price">NT$ {{ useNumberThousands(selectProduct.NowPrice) }}</div>
            </template>
            <template v-else>
              <template v-if="selectProduct.selectSpecItem && selectProduct.selectSpecItem.ID">
                <div class="price origin" v-if="parseInt(selectProduct.selectSpecItem.ItemPrice) > -1">NT$ {{ useNumberThousands(selectProduct.selectSpecItem.ItemPrice) }}</div>
                <div class="price">NT$ {{ useNumberThousands(selectProduct.selectSpecItem.ItemNowPrice) }}</div>
              </template>
              <template v-else>
            ...
            <div class="name"> <div v-html="useUnescapeHTML(selectProduct.Content)"></div> </div>
            ...
            <div class="content">
              ...
              <template v-else>
                <template v-if="item.selectSpecItem && item.selectSpecItem.ID">
                  <div class="price">NT$ {{ useNumberThousands(item.selectSpecItem.ItemNowPrice) }}</div>
                </template>
                <template v-else>
      ...
      <div class="detail">
        ...
        <div v-show="isDetail" class="content ql-editor" v-html="useUnescapeHTML(selectProduct.Detail)"></div>
      </div>
