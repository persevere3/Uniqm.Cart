        ...
        products.forEach(product => {
          if(product.priceType === 'multiPrice') {
            ...
            if(lowestPrice > 0 && highestPrice > 0) {        
              // 建議售價都一樣
              if(lowestPrice === highestPrice) product.priceRange = useNumberThousands(lowestPrice)
              else product.priceRange = `${useNumberThousands(lowestPrice)} - ${useNumberThousands(highestPrice)}`
            }
            ...
            // 售價都一樣
            if(lowestNowPrice === highestNowPrice) product.nowPriceRange = useNumberThousands(lowestNowPrice)
            else product.nowPriceRange = `${useNumberThousands(lowestNowPrice)} - ${useNumberThousands(highestNowPrice)}`
          }
        })
      ...
      try {
        ...
        addPrice.forEach(addPriceItem => {
          if(addPriceItem.PriceType === 'multiPrice') {
            ...
            let highestNowPrice = Math.max(...itemNowPriceArr)
            // 只有1規格 or 售價都一樣
            if(lowestNowPrice === highestNowPrice) addPriceItem.nowPriceRange = useNumberThousands(lowestNowPrice)
            else addPriceItem.nowPriceRange = `${useNumberThousands(lowestNowPrice)} - ${useNumberThousands(highestNowPrice)}`
          }
        })
